...
// TwilioOption customises the behaviour of the Twilio provider.
type TwilioOption func(*TwilioProvider)

// WithTwilioHTTPClient overrides the HTTP client used for outbound requests.
...
// WithTwilioBaseURL sets the Twilio API base URL. Primarily useful for tests.
...
// WithTwilioBodyLimit adjusts the maximum number of bytes retained from the response body.
...
// TwilioProvider implements the Provider interface using Twilio's REST API.
...
// NewTwilioProvider constructs a Twilio-backed SMS provider.
func NewTwilioProvider(cfg config.TwilioConfig, logger zerolog.Logger, opts ...TwilioOption) (*TwilioProvider, error) {
	if strings.TrimSpace(cfg.AccountSID) == "" {
		return nil, errors.New("twilio sms provider: account SID is required")
	}
	if strings.TrimSpace(cfg.AuthToken) == "" {
		return nil, errors.New("twilio sms provider: auth token is required")
	}
	if strings.TrimSpace(cfg.PhoneNumber) == "" {
		return nil, errors.New("twilio sms provider: phone number is required")
	}
	...
	provider := &TwilioProvider{
		...
		defaultFrom:  strings.TrimSpace(cfg.PhoneNumber),
...
// Send dispatches the SMS payload to Twilio.
func (p *TwilioProvider) Send(ctx context.Context, payload *Payload) (*RawResponse, error) {
	if payload == nil {
		return nil, errors.New("twilio sms provider: payload is required")
	}
	if len(payload.To) == 0 {
		return nil, errors.New("twilio sms provider: at least one recipient is required")
	}
	...
	if from == "" {
		return nil, errors.New("twilio sms provider: from number is required")
	}
...
func (p *TwilioProvider) sendSingle(ctx context.Context, endpoint, from, to string, payload *Payload) (*twilioResult, string, error) {
	params := url.Values{}
	params.Set("To", strings.TrimSpace(to))
	...
	for key, value := range payload.Meta {
		...
		if strings.EqualFold(key, "scenario") {
			// Reserved for mock provider testing.
			continue
		}
		if equalsIgnoreCase(key, "to") || equalsIgnoreCase(key, "from") || equalsIgnoreCase(key, "body") {
			continue
		}
	...
	if err != nil {
		return nil, "", fmt.Errorf("twilio sms provider: new request: %w", err)
	}
	...
	if err != nil {
		return nil, "", fmt.Errorf("twilio sms provider: http do: %w", err)
	}
	...
	if parsed.ErrorCode > 0 {
		return result, body, fmt.Errorf("twilio sms provider: error %d: %s", parsed.ErrorCode, message)
	}

	return result, body, fmt.Errorf("twilio sms provider: http %d: %s", resp.StatusCode, message)
}
...
func (p *TwilioProvider) readBody(rc io.ReadCloser) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("twilio sms provider: read body: %w", err)
	}
...
func parseTwilioBody(body string) twilioBody {
	...
	// Attempt to unmarshal when code/status are represented as strings.
